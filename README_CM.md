...
1. 确保 `package.json` 中的信息正确：
   ...
   - `version`: 版本号
   - `main`: 入口文件（`dist/release/opencode-prompt-log.js`）
   - `files`: 包含要发布的文件（`dist/`、`README.md`、`LICENSE`）

2. 确保 `README.md`、`README_CM.md` 和 `LICENSE` 文件存在
...
3. 确认 `dist/release/opencode-prompt-log.js` 文件已生成
